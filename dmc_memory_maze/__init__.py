...
try:
    ...
    for key, dm_task in sizes.items():
        ...
        register(id=f'MemoryMaze-{key}-Top-v0', entry_point=f(_make_gym_env, dm_task, top_camera=True))         # Top-down camera
        ...
        register(id=f'MemoryMaze-{key}-HiFreq-Top-v0', entry_point=f(_make_gym_env, dm_task, control_freq=40, top_camera=True))
        ...
        register(id=f'MemoryMaze-{key}-Img-Vis-v0', entry_point=f(_make_gym_env, dm_task, image_only_obs=True, good_visibility=True))
        register(id=f'MemoryMaze-{key}-Img-Top-v0', entry_point=f(_make_gym_env, dm_task, image_only_obs=True, top_camera=True))
        register(id=f'MemoryMaze-{key}-Img-HD-v0', entry_point=f(_make_gym_env, dm_task, image_only_obs=True, camera_resolution=256))
